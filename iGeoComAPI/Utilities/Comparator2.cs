...
namespace iGeoComAPI.Utilities
{
    public static class Comparator2
    {
        ...
        public static List<IGeoComDeltaModel> GetComparedResult(List<IGeoComGrabModel> newResult, List<IGeoComGrabModel> oldResult, params string[] ignoreList)
        {
            ...
            var modilfiedList = mergeModified(newModifiedWithState, oldModifiedWithState, ignoreList);
        ...
        public static List<IGeoComDeltaModel> mergeModified(List<IGeoComDeltaModel> newList, List<IGeoComDeltaModel> previousList, params string[] ignoreList)
        {
            var newMergeList = new List<IGeoComDeltaModel>();
            foreach (var n in newList)
            {
                newMergeList.Add(n);
                foreach(var p in previousList)
                {
                    List<bool> exist = GetShopCompareList(n, p, ignoreList);
                    if (exist.Contains(true))
                    {
                        newMergeList.Add(p);
                        break;
